...
const Chat = ([{ appointmentId, currentUserId }]) => {
    ...
    <div className="flex flex-col h-[500px] border border-slate-200 rounded-2xl overflow-hidden bg-white mt-8 shadow-sm">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50">
        {messages.map((m, i) => (
          ...
          >
            <div
              className={`max-w-[70%] rounded-2xl px-4 py-2 ${m.senderId === currentUserId ? "bg-blue-600 text-white rounded-br-none" : "bg-white border border-slate-200 text-slate-800 rounded-bl-none shadow-sm"}`}
            >
              {m.content}
            </div>
          </div>
